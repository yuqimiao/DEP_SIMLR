...
#   * Take in single data matrix/single distance matrix, give est_nclust of single data
#   * Take in list of data matrix/list of distance matrix, give est_nclust of integrated data(averaged)
Est_num_clus = function( X = NA,
                         distance = NA,
                         sigma = 2, allk = 30,
                         W = NA,
                         NUMC = 2:5,
  ...
  if(is.na(W)){
    if(!is.na(X)){
      if(use_former_kernel){
        if(is.list(X)){
          Kernels = unlist(lapply(X, function(x){
            multi_kernels_gl(x,cores.ratio = cores.ratio,sigma = sigma, allk = allk)
          }))
        }else{
          Kernels = multi_kernels_gl(x=X,cores.ratio = cores.ratio,sigma = sigma, allk = allk)
        }
        D_Kernels = lapply(Kernels,function(x){ # kernel to dist kernel
          dist_kernels(x)
        })
      }else{
        dk_tib = Dist_kernel_generation(sim = list(X),
                                        data_used = 1,
                                        sigma = sigma,
                                        allk = allk,
                                        diffusion_term = diff_term)
        D_Kernels = dk_tib$dist_kernel
      }
    }else if(!is.na(distance)){
      if(use_former_kernel){
        if(is.list(distance)){
          Kernels = unlist(lapply(distance, function(x){
            multi_kernels_gl(Diff = x,cores.ratio,sigma = sigma, allk = allk)
          }))
        }else{
          Kernels = multi_kernels_gl(Diff = distance,cores.ratio,sigma = sigma, allk = allk)
        }
        D_Kernels = lapply(Kernels,function(x){ # kernel to dist kernel
          dist_kernels(x)
        })
      }else{
        sim = list(dist_list = distance)
        dk_tib = Dist_kernel_generation(sim = sim,
                                        data_used = 1,
                                        sigma = sigma, allk = allk,
                                        diffusion_term = diff_term)
        D_Kernels = dk_tib$dist_kernel
      }
    }
  ...
  if(!is.null(NUMC)) {
    ...
    for (ck in NUMC) {
      if(ck==1) {
        ...
        EigenVectors = res$EigenVectors^2 ## Z(R)
